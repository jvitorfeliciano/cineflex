...
export default function Seats({
  ...
  function handleSeat(seatId, seatName) {
    ...
    if (seatStatus === false) {
      ...
      setSeatStatus(true);
      const auxOne = infosPurchase.seatsId.filter(
        (element) => element !== seatId
      );
      infosPurchase.seatsId = [...auxOne];
      const auxTwo = infosPurchase.seatsName.filter(
        (element) => element !== seatName
      );
